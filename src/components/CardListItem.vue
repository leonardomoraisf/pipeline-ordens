<template>
    <li>
                    ...
                    <div class="relative p-4">
                        <form class="p-3 rounded-md mt-2" @submit.prevent="onSubmit"
                            ...
                            <div class="mt-8 space-x-2 flex justify-end">
                                ...
                                <button type="submit"
                                    class="px-4 py-2 text-sm font-medium text-white bg-green-600/90 hover:bg-green-600/70 rounded-md shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-green-300 focus:outline-none">Salvar
                                    mudanças
                                </button>
                            </div>
                        </form>
        ...
        <div :style="{ backgroundColor: colorStatus }"
            class="group relative p-3 shadow rounded-md border-b border-gray-300 drag-card">

            <div class="h-28 hover:cursor-grab" :style="{ color: corTextoCard }">
                <p>{{ nomeCliente }}</p>
                <p>{{ precoMovimento }}</p>
                <p>{{ pedidoMovimento }}</p>
            </div>
            <button @click="showModal()" :style="{ backgroundColor: colorStatus }"
                class="hidden absolute top-1 right-1 w-8 h-8 group-hover:grid place-content-center rounded-md text-gray-900 hover:text-white hover:bg-gray-200">
...
export default {
    ...
    methods: {
        ...
        ajustarCorTexto(background) {
            ...
            this.corTextoCard = corTexto;
        }
    },
    mounted() {
        this.ajustarCorTexto(this.colorStatus);
    }
}
</script>

<style scoped> .drag>div {
