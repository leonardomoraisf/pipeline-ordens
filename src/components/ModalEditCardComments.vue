<template>
    <div>
            ...
            <div class="z-50 overflow-x-hidden overflow-y-auto flex justify-center items-center modal" v-if="toggleModal">
                <div class="relative mx-auto w-full">
                    <div class="bg-white w-full rounded-md h-screen p-3">
                        ...
                        <ul>
                            <CardListItem :card="cardIsEditing" :colorStatus="colorStatus" :corTextoCard="corTextoCard"
                                :dataHoje="dataHoje" :inModalEditComments="true"></CardListItem>
                        </ul>

                        <form class="mt-6" @submit.prevent="onSubmit">

                            <div v-if="!isSubmiting" class="relative">
                                <div class="absolute top-0 right-0">
                                    <p class="font-semibold text-sm text-gray-500"
                                        :class="{ 'text-red-500': cardComentarios.length > maxLength }">{{
                                            cardComentarios.length }} | máx: {{ maxLength }}</p>
                                </div>
                                <label for="message"
                                    class="block mb-2 text-sm font-medium text-gray-900 text-left">Comentários/Observações</label>
                                <textarea id="message" rows="10" name="comentarios" v-model="cardComentarios"
                                    ref="textareaRef"
                                ...
                                <div class="p-3 rounded-md mt-2">
                                    ...
                                    <div class="absolute bottom-0 right-0 p-3 flex justify-end">
                                        ...
                                        <div v-if="!isSubmiting">
                                            ...
                                            <button type="submit"
                                                class="px-8 py-4 ml-6 text-xl font-medium text-white salvar rounded-md shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-green-300 focus:outline-none hover:scale-110">Salvar</button>
                                        </div>
        ...
        <Transition name="fade" appear>
            <div class="absolute inset-0 z-40 bg-black/30" v-if="toggleModal"></div>
        </Transition>
    ...
    methods: {
        ...
        onSubmit(ev) {
            ...
            if (this.cardComentarios.length > this.maxLength) {
                ...
                this.errorMessage = 'O tamanho máximo dos comentários é de ' + this.maxLength + ' caracteres!';
            ...
            this.axios.put('/v2/pipeline/cards/' + this.cardIsEditing.id_card + '/edit', body)
                .then(res => {
